import xml.dom.minidom
import time
import os
import string, random
from pathlib import Path
from xml.etree import ElementTree

def customer_id_generator(size=128, chars=string.ascii_uppercase + string.digits):
	return ''.join(random.choice(chars) for _ in range(size))

def random_generator(size=10, chars=string.ascii_uppercase + string.digits):
	return ''.join(random.choice(chars) for _ in range(size))
	
	
def random_address_name_generator(size=8, chars=string.ascii_uppercase):
	return ''.join(random.choice(chars) for _ in range(size))
	
	
def random_address_street_generator(size=15, chars=string.ascii_uppercase + string.digits):
	return ''.join(random.choice(chars) for _ in range(size))
	
def random_address_city_generator(size=7, chars=string.ascii_uppercase ):
	return ''.join(random.choice(chars) for _ in range(size))
	
def random_address_state_generator(size=2, chars=string.ascii_uppercase ):
	return ''.join(random.choice(chars) for _ in range(size))
	
def random_address_zip_generator(size=5, chars=string.digits):
	return ''.join(random.choice(chars) for _ in range(size))
	
def random_address_country_generator(size=3, chars=string.ascii_uppercase):
	return ''.join(random.choice(chars) for _ in range(size))
	
	
	
# Random Date from https://stackoverflow.com/questions/553303/generate-a-random-date-between-two-other-dates
def str_time_prop(start, end, format, prop):
	"""Get a time at a proportion of a range of two formatted times.

	start and end should be strings specifying times formated in the
	given format (strftime-style), giving an interval [start, end].
	prop specifies how a proportion of the interval to be taken after
	start.  The returned time will be in the specified format.
	"""

	stime = time.mktime(time.strptime(start, format))
	etime = time.mktime(time.strptime(end, format))

	ptime = stime + prop * (etime - stime)

	return time.strftime(format, time.localtime(ptime))
	
def random_date(start, end, prop):
	return str_time_prop(start, end, '%m/%d/%Y %I:%M %p', prop)
# print(random_date("1/1/2008 1:30 PM", "1/1/2009 4:50 AM", random.random()))	
	
	
	
	
# generate random order id
def random_orderid_generator(size=5, chars=string.digits):
	return ''.join(random.choice(chars) for _ in range(size))
	
	
	
	
	

def main():

	for x in range(0, 8000):
		y = x
		# new process instance
		Path("./process_instances/%d" % x).mkdir(parents=True, exist_ok=True)

		# use the parse() function to load and parse an XML file
		doc = xml.dom.minidom.parse("offer_00_request_template_only_for_simulation.xml");
		
		
		customer_id_generator()
		CustomerID = doc.createElement("CustomerID")
		# CustomerID.setAttribute("Customer", customer_id_generator()  )
		text = doc.createTextNode( customer_id_generator() )
		CustomerID.appendChild(text)
		
		items = doc.getElementsByTagName("Address")
		item = items[0]
		item_chidren = item.childNodes
		item.insertBefore(CustomerID, item_chidren[0])
		
		# Add order date
		items_date = doc.getElementsByTagName("PurchaseOrder")
		items_date[0].setAttribute('OrderDate',  random_date("1/1/2008 1:30 PM", "1/1/2020 4:50 AM", random.random())  )
		
		
		# Add order ID
		items_date[0].setAttribute('PurchaseOrderNumber',  random_orderid_generator() )
		
		
		# +++++++++++++++++++ Address 1 / 2
		# +++++++++++++++++++
		# Add Address name
		address1_name = doc.createElement("Name")
		address1_name_text = doc.createTextNode( random_address_name_generator() )
		address1_name.appendChild(address1_name_text)
		doc.getElementsByTagName("Address")[0].appendChild(address1_name)
		# Street
		address1_street = doc.createElement("Street")
		address1_street_text = doc.createTextNode( random_address_street_generator() )
		address1_street.appendChild(address1_street_text)
		doc.getElementsByTagName("Address")[0].appendChild(address1_street)
		# City
		address1_city = doc.createElement("City")
		address1_city_text = doc.createTextNode( random_address_city_generator() )
		address1_city.appendChild(address1_city_text)
		doc.getElementsByTagName("Address")[0].appendChild(address1_city)
		# State
		address1_state = doc.createElement("State")
		address1_state_text = doc.createTextNode( random_address_state_generator() )
		address1_state.appendChild(address1_state_text)
		doc.getElementsByTagName("Address")[0].appendChild(address1_state)
		# Zip
		address1_zip = doc.createElement("Zip")
		address1_zip_text = doc.createTextNode( random_address_zip_generator() )
		address1_zip.appendChild(address1_zip_text)
		doc.getElementsByTagName("Address")[0].appendChild(address1_zip)
		# Country
		address1_country = doc.createElement("Country")
		address1_country_text = doc.createTextNode( random_address_country_generator() )
		address1_country.appendChild(address1_country_text)
		doc.getElementsByTagName("Address")[0].appendChild(address1_country)
		
		
		
		
		# +++++++++++++++++++ Address 2 / 2
		# +++++++++++++++++++
		# Add Address name
		address2_name = doc.createElement("Name")
		address2_name_text = doc.createTextNode( random_address_name_generator() )
		address2_name.appendChild(address2_name_text)
		doc.getElementsByTagName("Address")[1].appendChild(address2_name)
		# Street
		address2_street = doc.createElement("Street")
		address2_street_text = doc.createTextNode( random_address_street_generator() )
		address2_street.appendChild(address2_street_text)
		doc.getElementsByTagName("Address")[1].appendChild(address2_street)
		# City
		address2_city = doc.createElement("City")
		address2_city_text = doc.createTextNode( random_address_city_generator() )
		address2_city.appendChild(address2_city_text)
		doc.getElementsByTagName("Address")[1].appendChild(address2_city)
		# State
		address2_state = doc.createElement("State")
		address2_state_text = doc.createTextNode( random_address_state_generator() )
		address2_state.appendChild(address2_state_text)
		doc.getElementsByTagName("Address")[1].appendChild(address2_state)
		# Zip
		address2_zip = doc.createElement("Zip")
		address2_zip_text = doc.createTextNode( random_address_zip_generator() )
		address2_zip.appendChild(address2_zip_text)
		doc.getElementsByTagName("Address")[1].appendChild(address2_zip)
		# Country
		address2_country = doc.createElement("Country")
		address2_country_text = doc.createTextNode( random_address_country_generator() )
		address2_country.appendChild(address2_country_text)
		doc.getElementsByTagName("Address")[1].appendChild(address2_country)
		
		
		
		
		
		
		
		
		
		
		
		
		# create a new XML file with the results
		#myfile = open("offer_00_request.xml", "w")
		#myfile.write(doc.toprettyxml())
		with open("./process_instances/{0}/process_{0}_offer_00_request.xml".format( x ),  "w"  ) as xml_file:
			doc.writexml(xml_file)
		#time.sleep(5)
		
		
		
		
		
		
		
		#############################
		#############################
		#############################
		#############################
		doc = xml.dom.minidom.parse( "./process_instances/{0}/process_{0}_offer_00_request.xml".format( x )  );
		
		
		
		

		# print out the document node and the name of the first child tag
		print (doc.nodeName)
		print (doc.firstChild.tagName)
		# get a list of XML tags from the document and print each one
		expertise = doc.getElementsByTagName("Address")
		print ("%d expertise:" % expertise.length)
		for skill in expertise:
			print (skill.getAttribute("name"))

		# create a new XML tag and add it into the document
		newexpertise = doc.createElement("Supplierdata")
		newexpertise.setAttribute("Header_image", "data:image/png;base64,")
		doc.firstChild.appendChild(newexpertise)
		print (" ")
		
		# create a new XML tag and add it into the document
		NearestWarehouse = doc.createElement("NearestWarehouse")
		NearestWarehouse.setAttribute("GeoLocation", "333 Ravenswood Avenue Menlo Park, CA 94025")
		doc.firstChild.appendChild(NearestWarehouse)
		print (" ")

		expertise = doc.getElementsByTagName("Address")
		print ("%d expertise:" % expertise.length)
		for skill in expertise:
			print (skill.getAttribute("name"))

		# create a new XML file with the results
		#myfile = open("offer_01_request_with_address.xml", "w")
		#myfile.write(doc.toprettyxml())
		with open("./process_instances/{0}/process_{0}_offer_01_request_with_address.xml".format(x), "w"  ) as xml_file:
			doc.writexml(xml_file)
		#time.sleep(5)
		
		
		
		
		
		
		
		#############################
		#############################
		#############################
		#############################
		doc = xml.dom.minidom.parse("./process_instances/{0}/process_{0}_offer_01_request_with_address.xml".format(x) );

		# print out the document node and the name of the first child tag
		print (doc.nodeName)
		print (doc.firstChild.tagName)
		# get a list of XML tags from the document and print each one
		expertise = doc.getElementsByTagName("Address")
		print ("%d expertise:" % expertise.length)
		for skill in expertise:
			print (skill.getAttribute("name"))

		# create a new XML tag and add it into the document
		pricelist = doc.createElement("Prices")
		pricelist.setAttribute("p1", "price 1")
		pricelist.setAttribute("p2", "price 2")
		pricelist.setAttribute("p3", "price 3")
		pricelist.setAttribute("p4", "price 4")
		pricelist.setAttribute("p5", "price 5")
		# doc.firstChild.appendChild(pricelist)
		print (" ")

		expertise = doc.getElementsByTagName("Items")
		print ("%d expertise:" % expertise.length)
		for item in expertise:
			print (item.getAttribute("name"))
			item.appendChild(pricelist)

		# create a new XML file with the results
		#myfile = open("offer_02_request_with_address_and_prices.xml", "w")
		#myfile.write(doc.toprettyxml())
		with open("./process_instances/{0}/process_{0}_offer_02_request_with_address_and_prices.xml".format(x) , "w") as xml_file:
			doc.writexml(xml_file)
		


		#time.sleep(5)
		
		
		#############################
		#############################
		#############################
		#############################
		doc = xml.dom.minidom.parse("./process_instances/{0}/process_{0}_offer_02_request_with_address_and_prices.xml".format(x) );

		# print out the document node and the name of the first child tag
		print (doc.nodeName)
		print (doc.firstChild.tagName)
		# get a list of XML tags from the document and print each one
		expertise = doc.getElementsByTagName("Address")
		print ("%d expertise:" % expertise.length)
		for skill in expertise:
			print (skill.getAttribute("name"))

		# create a new XML tag and add it into the document
		newexpertise = doc.createElement("Comments")
		newexpertise.setAttribute("c1", "asdfl-kaskölfdjhakjsdfhlkasjdfhiuakshdfliuaksjhföoasi4hö89gp4huaöofihe4öohifsaöiyg4uhofesöysgirauörosgihauöshifhaösoyifhoshöeifjesoirjgsoetrgsdrgsdrg")
		newexpertise.setAttribute("c2", "p98segp9749za9gzp4p9zg4aöiuagrklsdrkjgshkurghiöusjekrhögiushrögioslhydrögoisehögiseydorgiseydögsihöeiglsoöeighosheiglhoösitrjgosghtidjgdohöitjiotidthjothidjgoihts")
		newexpertise.setAttribute("c3", "saiofhu9ae4ige48z9t0q3z4gh0a8wotuö4g8hesoa4z8öghsöoeö48hrhusögeos4irsöirhgösoeidrhgsöoeirhgöoseihrgöosueihrgöosuehrgosuerhgöoierhtgoiöehörogihseo5rithgisoerhtgoeir")
		newexpertise.setAttribute("c4", "dgiuhayilsughaiw4g948pa8w4z984gzö98az4wt98qz4piuserzughesrgliuhserögloseöuorildghseiurlsegirishuöesirhösuöhiskjdruszrsieruzsriusjkhrsökhö")
		doc.firstChild.appendChild(newexpertise)
		print (" ")

		expertise = doc.getElementsByTagName("Address")
		print ("%d expertise:" % expertise.length)
		for skill in expertise:
			print (skill.getAttribute("name"))

		# create a new XML file with the results
		#myfile = open("./process_instances/%d/offer_03_request_with_address_and_prices_and_comments.xml" % x, "w")
		#myfile.write(doc.toprettyxml())
		with open("./process_instances/{0}/process_{0}_offer_03_request_with_address_and_prices_and_comments.xml".format(x) , "w") as xml_file:
			doc.writexml(xml_file)
		
		
		
		#y = random_generator()
		#os.rename(r'./process_instances/%d/offer_00_request.xml',r'./process_instances/%d/offer_00_request_{y}.xml' % x )
		
if __name__ == "__main__":
	main();